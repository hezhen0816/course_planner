...
- 正式資料在 Supabase 專案 `course-compass`（ref `qpdvtsbqdpitreslazoe`，免費方案，閒置約一週會被暫停，需到 Dashboard 按 Restore）。學分規劃在 `public.user_data`，校務帳密與官方 session 密文在 `app_private.*`，同步快照在 `*_snapshots`。
