...
- 學校端行為（SSO 進入點、名額欄位語意、通識課碼、選課階段）查證結果在 [docs/data-contracts/ntust-systems.md](docs/data-contracts/ntust-systems.md)，改相關程式前先讀，不要憑印象重推。
- 正式資料在 Supabase 專案 `course-compass`（ref `eerlhmvwucnlbhemhvtz`，2026-09-08 自 `NTUST_Course_Monitor_restored` 改名；舊的 `qpdvtsbqdpitreslazoe` 專案已不存在）。schema 以 `supabase/migrations/` 為準（含 Monitor 的三張表），舊專案的歷史 migration 在 `docs/archive/supabase-migrations-old-project/` 只讀。學分規劃在 `public.user_data`，校務帳密與官方 session 密文在 `app_private.*`，同步快照在 `*_snapshots`。
- 後端、Web 與課程監控 worker（`backend/monitor/`，2026-09-08 自 NTUST_Course_Monitor 搬入）都跑在家用 Windows 主機，經 Tailscale 提供 HTTPS；網址、部署腳本與排程器名稱見 README「Windows 後端部署」。Web 的正式入口是 Vercel `https://ntust-course-compass.vercel.app`（push `main` 自動部署；Vercel 頁面呼叫後端仍需瀏覽器在 tailnet 內）；`course-compass.vercel.app` 是別人的網域。Railway 專案已於 2026-09-08 全部刪除，`backend/Procfile`、`nixpacks.toml` 已移除。NTUST_Course_Monitor 的 Vercel 站與 Windows 排程工作已於 2026-09-08 刪除，GitHub repo 已封存（只讀）；Windows 上舊 checkout 移到 `C:\Users\hezhe\source\repos\_retired\`。
